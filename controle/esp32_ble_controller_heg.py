...
class FeedbackController:
    """Controlador para integração com endpoint de feedback HTTP"""
    ...
    @staticmethod
    def obter_dados_raw_feedback(url="http://localhost:5001/Feedback/RawData/200"):
    ...
    @staticmethod
    def calc_avg(tmp):
        """Calcula média dos dados de feedback"""
    ...
    @staticmethod
    def obter_valor_raw_feedback(tmp):
        """Extrai o último valor numérico dos dados de feedback (sem calcular média).

        Suporta formatos comuns retornados pelo endpoint: lista de valores,
        lista de listas/tuplas (cada item contendo um valor), ou lista de dicts.
        Retorna None se não for possível extrair um valor numérico.
        """
        if tmp is None or len(tmp) == 0:
            return None
        ...
        try:
            ...
            # Se for lista/tupla, pega o primeiro elemento
            if isinstance(last, (list, tuple)) and len(last) > 0:
                val = last[0]

            # Se for dict, tenta extrair um campo óbvio ou o primeiro valor
            elif isinstance(last, dict):
                # tenta chaves comuns
                for k in ("value", "valor", "v", "val"):
                    if k in last:
                        val = last[k]
                        break
                else:
                    # pega o primeiro valor do dict
                    val = next(iter(last.values()))

            else:
                # Pode ser número diretamente
                val = last

            return float(val)
        except Exception as e:
            print(f"Erro ao extrair valor raw do feedback: {e}")
            return None
    
    def start_feedback_control(self, controller, threshold=540, period_ms=10000, duration_s=30, callback=None):
        """Inicia controle baseado em feedback"""
        if self.running:
            print("⚠️ Controle por feedback já está rodando")
            return False
            
        self.running = True
        self.thread = threading.Thread(
            target=self._feedback_loop, 
            args=(controller, threshold, period_ms, duration_s, callback),
            daemon=True
        )
        self.thread.start()
        return True
    
    def stop_feedback_control(self):
        """Para o controle por feedback"""
        self.running = False
        if self.thread:
            self.thread.join(timeout=2)
    
    def _feedback_loop(self, controller, threshold, period_ms, duration_s, callback):
        """Loop principal de controle por feedback"""
        trial = 0
        trial_limit = int(duration_s / (period_ms / 1000))
        threshold_triggered = False
        
        while self.running and trial < trial_limit:
            # Obtém dados do feedback
            tmp = self.obter_dados_raw_feedback()

            # Usa o último valor bruto (sem média) conforme desejo do usuário
            valor = self.obter_valor_raw_feedback(tmp)

            if callback:
                # callback recebe (trial, valor, threshold)
                callback(trial, valor if valor is not None else 0, threshold)

            if valor is None:
                print(f"Trial {trial}/{trial_limit}: sem dados válidos, assumindo 0 (threshold={threshold})")
                # Tenta solicitar novamente mais tarde, não modifica trial para dar chance
                controller.send_command("CMD:FOLLOW_LINE_START")
                time.sleep(period_ms / 1000.0)
                continue

            print(f"Trial {trial}/{trial_limit}: valor={valor:.2f}, threshold={threshold}")

            # Decide ação baseado no threshold (compara diretamente o valor bruto)
            if valor > threshold:
                # Quando atinge threshold, enviamos STOP (uma vez) mas não encerramos o controle
                if not threshold_triggered:
                    print(f"🛑 Threshold atingido ({valor:.2f} > {threshold}) - enviando STOP, continuará monitorando até o fim da duração")
                    controller.send_command("CMD:FOLLOW_LINE_STOP")
                    threshold_triggered = True
                else:
                    print(f"🔁 Threshold já atingido anteriormente; monitorando (valor={valor:.2f} > {threshold})")
                # Não incrementa trial quando threshold já foi atingido; apenas espera próximo ciclo
                trial += 1
            else:
                # Se ainda não houve trigger, mantém o seguidor ativo
                if not threshold_triggered:
                    controller.send_command("CMD:FOLLOW_LINE_START")
                else:
                    # Se já houve trigger, mantém STOP (não reinicia automaticamente)
                    pass
                trial += 1
        ...
        # Para o seguidor ao final
        controller.send_command("CMD:FOLLOW_LINE_STOP")
        if callback:
            callback(-1, 0, threshold)  # Sinal de término
        print("✅ Controle por feedback finalizado")
...
class RobotGUI:
    def __init__(self, root):
        ...
        # Inicializa o controlador de feedback
        ...
        # Variáveis do controle por feedback
        self.feedback_threshold = tk.DoubleVar(value=540)
        self.feedback_period_ms = tk.IntVar(value=10000)
        self.feedback_duration_s = tk.IntVar(value=30)
    ...
    def create_widgets(self):
        ...
        # Aba 3: Configurações (comandos do sistema e marcadores)
    ...
    def create_advanced_frame(self, parent):
        ...
        # Marcadores de linha
    # (Os controles de 'Comandos do Sistema' e 'Marcadores' foram movidos
    # para a aba '⚙️ Configurações' para melhorar organização.)
        
        # Controle por Feedback HTTP
        ttk.Separator(advanced_frame, orient="horizontal").grid(row=16, column=0, sticky=(tk.W, tk.E), pady=5)
        
        ttk.Label(advanced_frame, text="🧠 Controle por Feedback", font=('TkDefaultFont', 9, 'bold')).grid(row=17, column=0, pady=(5,2))
        
        # Threshold
        threshold_frame = ttk.Frame(advanced_frame)
        threshold_frame.grid(row=18, column=0, padx=5, pady=2, sticky=(tk.W, tk.E))
        ttk.Label(threshold_frame, text="Threshold:").pack(side=tk.LEFT, padx=(0, 5))
        threshold_spin = ttk.Spinbox(threshold_frame, from_=100, to=1000, increment=10, width=6,
                                    textvariable=self.feedback_threshold, format="%.0f")
        threshold_spin.pack(side=tk.LEFT)
        
        # Período
        period_frame = ttk.Frame(advanced_frame)
        period_frame.grid(row=19, column=0, padx=5, pady=2, sticky=(tk.W, tk.E))
        ttk.Label(period_frame, text="Período (ms):").pack(side=tk.LEFT, padx=(0, 5))
        period_spin = ttk.Spinbox(period_frame, from_=1000, to=30000, increment=1000, width=6,
                                 textvariable=self.feedback_period_ms, format="%.0f")
        period_spin.pack(side=tk.LEFT)
        
        # Duração
        duration_frame = ttk.Frame(advanced_frame)
        duration_frame.grid(row=20, column=0, padx=5, pady=2, sticky=(tk.W, tk.E))
        ttk.Label(duration_frame, text="Duração (s):").pack(side=tk.LEFT, padx=(0, 5))
        duration_spin = ttk.Spinbox(duration_frame, from_=10, to=300, increment=10, width=6,
                                   textvariable=self.feedback_duration_s, format="%.0f")
        duration_spin.pack(side=tk.LEFT)
        
        self.feedback_btn = ttk.Button(advanced_frame, text="🧠 Iniciar Controle", 
                                      command=self.toggle_feedback_control)
        self.feedback_btn.grid(row=21, column=0, padx=5, pady=2, sticky=(tk.W, tk.E))
        
        # Display de status do feedback
        self.feedback_status = ttk.Label(advanced_frame, text="Aguardando...", foreground="gray")
        self.feedback_status.grid(row=22, column=0, padx=5, pady=2)
        
        # Telemetria
        ttk.Separator(advanced_frame, orient="horizontal").grid(row=23, column=0, sticky=(tk.W, tk.E), pady=5)
        
        # Controle de taxa de atualização
        hz_frame = ttk.Frame(advanced_frame)
        hz_frame.grid(row=24, column=0, padx=5, pady=2, sticky=(tk.W, tk.E))
        ...
        self.telemetry_btn.grid(row=25, column=0, padx=5, pady=2, sticky=(tk.W, tk.E))
        
        # Display de telemetria
        self.telemetry_text = scrolledtext.ScrolledText(advanced_frame, width=30, height=8)
        self.telemetry_text.grid(row=26, column=0, padx=5, pady=5, sticky=(tk.W, tk.E, tk.N, tk.S))
        
        advanced_frame.columnconfigure(0, weight=1)
        advanced_frame.rowconfigure(26, weight=1)

    def create_config_tab(self, notebook):
        """Cria a aba de configurações com comandos do sistema e marcadores"""
        config_tab = ttk.Frame(notebook)
        notebook.add(config_tab, text="⚙️ Configurações")

        main_frame = ttk.Frame(config_tab, padding="10")
        main_frame.pack(fill=tk.BOTH, expand=True)

        # Comandos do sistema
        system_frame = ttk.LabelFrame(main_frame, text="Comandos do Sistema", padding="5")
        system_frame.pack(fill=tk.X, padx=5, pady=5)

        ttk.Button(system_frame, text="START", 
                  command=lambda: self.send_command("CMD:START")).pack(fill=tk.X, pady=2)
        ttk.Button(system_frame, text="RESET_KALMAN", 
                  command=lambda: self.send_command("CMD:RESET_KALMAN")).pack(fill=tk.X, pady=2)
        ttk.Button(system_frame, text="CALIBRATE_IMU", 
                  command=lambda: self.send_command("CMD:CALIBRATE_IMU")).pack(fill=tk.X, pady=2)

        # Marcadores
        markers_frame = ttk.LabelFrame(main_frame, text="🎯 Marcadores", padding="5")
        markers_frame.pack(fill=tk.X, padx=5, pady=5)

        ttk.Button(markers_frame, text="🔄 Reset Marcadores", 
                  command=self.reset_markers).pack(fill=tk.X, pady=2)

        # Configuração de espaçamento
        spacing_frame = ttk.Frame(markers_frame)
        ...
        ttk.Button(markers_frame, text="⚙️ Aplicar Espaçamento", 
                  command=self.apply_marker_spacing).pack(fill=tk.X, pady=2)
    ...
    def toggle_feedback_control(self):
        """Inicia/para o controle por feedback HTTP"""
        if not self.controller.is_connected:
            messagebox.showwarning("Aviso", "Conecte-se ao ESP32 primeiro")
            return
        
        if not self.feedback_controller.running:
            # Inicia controle por feedback
            threshold = self.feedback_threshold.get()
            period_ms = self.feedback_period_ms.get()
            duration_s = self.feedback_duration_s.get()
            
            success = self.feedback_controller.start_feedback_control(
                self.controller,
                threshold=threshold,
                period_ms=period_ms,
                duration_s=duration_s,
                callback=self.on_feedback_update
            )
            
            if success:
                self.feedback_btn.config(text="⏹️ Parar Controle")
                self.feedback_status.config(text="🔄 Controle ativo...", foreground="blue")
                self.log_message(f"🧠 Controle por feedback iniciado (threshold={threshold}, período={period_ms}ms, duração={duration_s}s)")
        else:
            # Para controle por feedback
            self.feedback_controller.stop_feedback_control()
            self.feedback_btn.config(text="🧠 Iniciar Controle")
            self.feedback_status.config(text="⏹️ Controle parado", foreground="red")
            self.log_message("🧠 Controle por feedback parado")
    
    def on_feedback_update(self, trial, mu, threshold):
        """Callback para atualizar status do feedback"""
        if trial == -1:
            # Sinal de término
            self.root.after(0, lambda: self.feedback_status.config(
                text="✅ Controle finalizado", foreground="green"))
            self.root.after(0, lambda: self.feedback_btn.config(text="🧠 Iniciar Controle"))
        else:
            # Atualização normal
            status_text = f"Trial {trial}: μ={mu:.2f}"
            if mu > threshold:
                status_text += " 🛑"
                color = "red"
            else:
