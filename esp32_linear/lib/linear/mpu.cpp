...
void LinearCar::update_mpu() {
    if (!this->mpu_available) return;
    if (mpu.update()) {
        this->correct_imu();
    }
}
void LinearCar::calibrate_imu() {
    if (!this->mpu_available) return;
 ...
 */
void LinearCar::calibrate_gyro(uint8_t loops) {
    float kP = 0.3;
    float kI = 90;
 ...
 */
void LinearCar::calibrate_accel(uint8_t loops) {
    float kP = 0.3;
    float kI = 20;
    float x = (100 - map(loops, 1, 5, 20, 0)) * 0.01;
    kP *= x;
...
void LinearCar::calibrate_magnetometer(uint8_t loops) {
    float kP = 0.3;
    float kI = 20;
    float x = (100 - map(loops, 1, 5, 20, 0)) * 0.01;
    kP *= x;
 ...
 */
void LinearCar::pid_calibration(uint8_t readAddress, float kP, float kI, uint8_t loops) {
